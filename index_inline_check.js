...
// ------------------ details 제거 ------------------
// 접힘 영역은 DOM으로 파싱된 뒤에는 브라우저가 구조를 보정하거나
// extractLikelyChatHTML 단계에서 바깥 <details> 껍데기가 잘릴 수 있습니다.
// 그래서 파일/붙여넣기 원본 HTML 문자열 단계에서 먼저 지웁니다.
function stripDetailsRawHTML(html){
  let s = String(html || "");
  if(!s) return s;

  // 1. 정상적으로 닫힌 <details>...</details> 블록 전체 삭제
  let prev = null;
  while(prev !== s){
    prev = s;
    s = s.replace(/<details\b[^>]*>[\s\S]*?<\/details\s*>/gi, "");
  }

  // 2. 한 응답 셀 안에서 <details>가 열렸지만 닫히지 않은 경우:
  //    <details> 시작부터 현재 셀의 끝으로 보이는 지점까지 삭제
  //    rofan-block / 일반 블록 닫힘 / 다음 rofan-block / 문서 끝을 경계로 봅니다.
  s = s.replace(/<details\b[^>]*>[\s\S]*?(?=<div\b[^>]*class=["'][^"']*rofan-block|<\/(?:div|article|section|td|li|main)\s*>|$)/gi, "");

  // 남은 summary 조각도 제거
  s = s.replace(/<summary\b[^>]*>[\s\S]*?<\/summary\s*>/gi, "");
  s = s.replace(/<\/?(?:details|summary)\b[^>]*>/gi, "");
  return s;
}
function shouldStripDetailsNow(){
  return !!(removeDetailsEl && removeDetailsEl.checked);
}
function stripDetailsForCurrentOption(html){
  return shouldStripDetailsNow() ? stripDetailsRawHTML(html) : String(html || "");
}
function filterDetailsByProtection(text, tokens){
  return stripDetailsRawHTML(text || "");
}
...
async function loadChatFile(){
  ...
  try{
    ...
    const looksHTML = /<!doctype\s+html\b|<html\b|<body\b|<(div|p|span|details)\b/i.test(raw);
    ...
    if(looksStructuredMarkdown){
      ...
      const detailFilteredHtml = stripDetailsForCurrentOption(html);
      if(!detailFilteredHtml || !looksLikeHTML(detailFilteredHtml)){
        showToast("파일 안에서 HTML 채팅 내용을 찾지 못했습니다.");
        return;
      }
      sanitized = sanitizePastedHTML(extractLikelyChatHTML(detailFilteredHtml));
    }else{
...
function handleRichPaste(e){
  ...
  if(html){
    e.preventDefault();
    resetReviewDecisions();
    insertHtmlAtCursor(sanitizePastedHTML(stripDetailsForCurrentOption(html)));
    transformText();
    return;
  }
...
function sanitizePastedHTML(html){
  html = stripDetailsForCurrentOption(html);
  const doc = new DOMParser().parseFromString(html || "", "text/html");
  ...
  const pushChunksFromElement = (el, baseOwner) => {
    ...
    const walk = (node, ctx) => {
      ...
      const nextCtx = {
        ...
        fromDetails: ctx.fromDetails || childEl.tagName === "DETAILS" || !!(childEl.closest && childEl.closest("details")),
    ...
    Array.from(el.childNodes).forEach(child => walk(child, {
      ...
      fromDetails: el.tagName === "DETAILS" || !!(el.closest && el.closest("details")),
...
function parseHtmlToRofanChunks(html){
  ...
  const filteredHtml = stripDetailsForCurrentOption(String(html || ''));
  holder.innerHTML = sanitizePastedHTML(extractLikelyChatHTML(filteredHtml));
