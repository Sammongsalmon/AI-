...
(function(){
  ...
  async function runRofanExporter(){
    ...
    try {
      ...
      var html = buildStructuredHTML(all, title, chatId);
  ...
  function buildStructuredHTML(logs, title, chatId){
    var parts = [];
    for (var i = 0; i < logs.length; i++) {
      var log = logs[i] || {};
      var blockBase = safeMeta(log.log_id || log.pk || ("log-" + (i + 1)));
