...
(function(){
  ...
  async function runRofanExporter(){
    ...
    try {
      ...
      var firstMessage = getRofanFirstMessage(pageProps);
  ...
  function getRofanFirstMessage(pageProps){
    var text = (
      ...
      pageProps.bot && pageProps.bot.first_message ||
      ""
    );
  ...
  function sameLooseText(a, b){
    return cleanText(a).replace(/\s+/g, "") === cleanText(b).replace(/\s+/g, "");
  }
  ...
  function buildStructuredHTML(logs, title, chatId, firstMessage){
    ...
    if (firstText && !sameLooseText(firstText, firstBotText)) {
