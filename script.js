...
/* --- hardening patch: file loading, rendered input, safe transforms, tab-local chapters --- */
(function(){
  if(window.__rofanHardeningV3Applied) return;
  window.__rofanHardeningV3Applied = true;

  const CHAPTER_MARK = "\uE000ROFAN_CHAPTER_BREAK\uE001";
  window.ROFAN_CHAPTER_BREAK_TOKEN = CHAPTER_MARK;
  const $ = id => document.getElementById(id);
  const safe = (fn, fallback) => { try { return fn(); } catch(err){ console.error(err); return fallback; } };
  const esc = s => (typeof escapeHTML === "function" ? escapeHTML(s) : String(s||"").replace(/[&<>\"]/g, ch => ({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[ch])));
  const attrEsc = s => (typeof escapeAttr === "function" ? escapeAttr(s) : esc(s).replace(/`/g,"&#096;"));
  const stripTagsSafe = s => (typeof stripHTMLTags === "function" ? stripHTMLTags(s) : String(s||"").replace(/<[^>]*>/g,""));
  const markRe = () => new RegExp(CHAPTER_MARK.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g");

  function injectHardeningCss(){
    if($("rofanHardeningV3Style")) return;
    const style = document.createElement("style");
    style.id = "rofanHardeningV3Style";
    style.textContent = `
      .rawInputStorage{display:none!important;}
      .renderedInputPreview{display:block!important;min-height:520px;height:520px;}
      label.prettyFile,.fileControl{position:relative!important;}
      label.prettyFile input[type="file"],.fileControl input[type="file"]{position:absolute!important;inset:0!important;inline-size:100%!important;block-size:100%!important;opacity:0!important;overflow:hidden!important;cursor:pointer!important;}
      .renderedInputPreview .rofan-block{margin:0 0 1em;}
      .renderedInputPreview .rofan-block p{margin:0 0 .72em;}
      .richResultBox .chapter-editor-separator,.renderedInputPreview .chapter-editor-separator,.epubRichEditor .chapter-editor-separator{
        border:0;border-top:2px dashed var(--line);height:0;margin:18px 0;position:relative;display:block;width:100%;
      }
      .richResultBox .chapter-editor-separator::after,.renderedInputPreview .chapter-editor-separator::after,.epubRichEditor .chapter-editor-separator::after{
        content:"챕터 구분선";position:relative;top:-12px;display:inline-block;padding:2px 8px;border:1px solid var(--line);border-radius:999px;background:var(--paper);color:var(--muted);font-size:11px;
      }
      .chapterTocBox{display:grid;gap:8px;}
      .chapterTocRow{display:grid;grid-template-columns:34px minmax(0,1fr) auto;gap:9px;align-items:center;padding:8px 0;border-bottom:1px solid var(--line2);}
      .chapterTocRow b{width:24px;height:24px;border-radius:8px;background:var(--paper2);display:grid;place-items:center;color:var(--sub);font-size:11px;}
      .chapterTocRow span{min-width:0;display:block;overflow:hidden;}
      .chapterTocRow strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:700;color:rgba(31,45,54,.78);}
      .chapterTocRow small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--muted);font-size:12px;margin-top:2px;}
      .chapterTocRow em{font-style:normal;color:var(--muted);font-size:12px;white-space:nowrap;}
      .tocSummary{display:flex;justify-content:space-between;gap:10px;align-items:center;color:var(--sub);font-size:13px;border-bottom:1px solid var(--line2);padding-bottom:6px;margin-bottom:2px;}
    `;
    document.head.appendChild(style);
  }

  function htmlFromVisibleText(text){
    if(typeof htmlFromResultText === "function") return htmlFromResultText(String(text || ""));
    return esc(text || "");
  }
  const oldHtmlFromResultText = (typeof htmlFromResultText === "function") ? htmlFromResultText : null;
  window.htmlFromResultText = htmlFromResultText = function(text){
    const raw = String(text || "");
    const html = oldHtmlFromResultText ? oldHtmlFromResultText(raw) : esc(raw);
    const escapedMark = esc(CHAPTER_MARK);
    return html.replace(new RegExp(escapedMark, "g"), '<hr class="chapter-editor-separator" data-chapter-separator="true">');
  };

  function ensureClassicInputPreview(){
    const input = $("inputText");
    if(!input) return null;
    let prev = $("inputTextPreview");
    if(!prev){
      prev = document.createElement("div");
      prev.id = "inputTextPreview";
      prev.className = "pastebox renderedInputPreview";
      prev.contentEditable = "true";
      prev.spellcheck = false;
      prev.setAttribute("role", "textbox");
      prev.setAttribute("aria-label", "원본 로그");
      prev.dataset.placeholder = input.getAttribute("placeholder") || "여기에 로그를 붙여 넣으세요";
      input.insertAdjacentElement("afterend", prev);
    }
    input.classList.add("rawInputStorage");
    if(!prev.dataset.boundHardening){
      prev.dataset.boundHardening = "1";
      prev.addEventListener("input", () => {
        try{ classicActiveChunks = null; classicActiveDisplayText = ""; }catch(_err){}
        input.value = (prev.innerText || prev.textContent || "").replace(/\u00a0/g, " ").trim();
        robustTransformText();
        if(typeof scheduleAutosave === "function") scheduleAutosave();
      });
      prev.addEventListener("paste", e => {
        e.preventDefault();
  ...
  function chunkToRenderItem(c, idx){
    return {blockId:c.blockId || ("block-"+idx), owner:c.owner || "character", kind:c.kind || "normal", text:c.text || ""};
  }
  function structuredChunksToDisplayHtml(chunks){
    if(typeof structuredItemsToHtml === "function") return structuredItemsToHtml((chunks || []).map(chunkToRenderItem));
    return (chunks || []).map(c => `<p>${htmlFromVisibleText(c.text || "")}</p>`).join("\n");
  }
  function renderClassicInputFromText(text, chunks){
    const prev = ensureClassicInputPreview();
    if(!prev) return;
    const old = prev.scrollTop;
    let html = "";
    if(chunks && chunks.length) html = structuredChunksToDisplayHtml(chunks);
    else {
      const raw = String(text || "");
      const parsed = safe(() => parseStructuredMarkdownToChunks(raw), null);
      if(parsed && parsed.length) html = structuredChunksToDisplayHtml(parsed);
      else html = htmlFromVisibleText(raw.replace(/<!--\s*rofan:[\s\S]*?-->/gi, "").replace(markRe(), CHAPTER_MARK));
    }
    prev.innerHTML = html;
    prev.scrollTop = old;
  }

  // Markdown 구조 파서 보강: 첫 rofan 주석 앞의 첫 문단도 버리지 않습니다.
  window.parseStructuredMarkdownToChunks = parseStructuredMarkdownToChunks = function(raw){
    const text = String(raw || "").replace(/\r/g, "");
    const re = /<!--\s*rofan:([^>]*)-->/gi;
    const records = [];
    let match, last = 0, current = {owner:"character", kind:"scene", block:"md-prelude"};
    let sawMarker = false;
    function pushBody(body, meta){
      const clean = String(body || "").trim();
      if(!clean) return;
      records.push({owner:meta.owner || "character", kind:meta.kind || "normal", block:meta.block || ("md-" + records.length), body:clean});
    }
    while((match = re.exec(text))){
      const between = text.slice(last, match.index);
      if(sawMarker) pushBody(between, current);
      else pushBody(between, {owner:"character", kind:"scene", block:"md-prelude"});
      const attrs = (typeof parseRofanCommentAttrs === "function") ? parseRofanCommentAttrs(match[1] || "") : {};
      const owner = (attrs.owner || "character").trim() || "character";
      const kind = (attrs.kind || "normal").trim() || "normal";
      const block = (attrs.block || attrs.blockid || attrs["block-id"] || "").trim() || `md-${records.length}`;
      current = {owner, kind, block};
      sawMarker = true;
      last = re.lastIndex;
    }
    if(sawMarker) pushBody(text.slice(last), current);
    if(!records.length) return null;
    const chunks = [];
    let seq = 0;
    records.forEach((rec, recIdx) => {
      String(rec.body || "").split(/\n\s*\n+/).map(p=>p.trim()).filter(Boolean).forEach(p => {
        chunks.push({id:"mdc-"+(seq++), kind:rec.kind || "normal", owner:rec.owner || "character", text:p, color:"", fromTable:false, fromDetails:false, blockId:rec.block || ("md-"+recIdx)});
      });
    });
    return chunks.length && typeof normalizeChunkResponseBlocks === "function" ? normalizeChunkResponseBlocks(chunks) : (chunks.length ? chunks : null);
  };
  window.structuredMarkdownToHtml = structuredMarkdownToHtml = function(raw){
    const chunks = parseStructuredMarkdownToChunks(raw);
    return chunks && chunks.length ? structuredChunksToDisplayHtml(chunks) : "";
  };

  function updateFileLabel(inputEl, labelEl, empty){
    if(!labelEl) return;
    const f = inputEl && inputEl.files && inputEl.files[0];
    labelEl.textContent = f ? (f.name || "선택됨") : (empty || "선택된 파일 없음");
  }
  async function readTextFileSafe(file){
    if(typeof readFileAsTextSmart === "function") return readFileAsTextSmart(file);
    const buf = await file.arrayBuffer();
    return new TextDecoder("utf-8").decode(buf).replace(/^\uFEFF/, "");
  }
  function contentToChunksSafe(raw, name){
    if(typeof fileContentToChunks === "function"){
      const chunks = safe(() => fileContentToChunks(raw, name), null);
      if(chunks && chunks.length) return chunks;
    }
    if(/<!--\s*rofan:/i.test(raw) || /\.(md|markdown)$/i.test(name || "")){
      const chunks = parseStructuredMarkdownToChunks(raw);
      if(chunks && chunks.length) return chunks;
    }
    return null;
  ...
  window.loadClassicFile = loadClassicFile = async function(){
    const fileInput = $("classicFileInput");
    const file = fileInput && fileInput.files && fileInput.files[0];
    if(!file) return;
    try{
      updateFileLabel(fileInput, $("classicFileName"));
      const raw = await readTextFileSafe(file);
      const chunks = contentToChunksSafe(raw, file.name || "");
      if(chunks && chunks.length){
        classicLoadedChunks = chunks;
        classicLoadedFileText = (typeof chunksDisplayText === "function") ? chunksDisplayText(chunks) : chunks.map(c=>c.text).join("\n\n");
      }else{
        classicLoadedChunks = null;
        classicLoadedFileText = String(raw || "");
      }
      applyClassicExcerpt();
      renderClassicInputFromText($("inputText") ? $("inputText").value : "", classicActiveChunks && classicActiveChunks.length ? classicActiveChunks : classicLoadedChunks);
      robustTransformText();
      if(typeof showToast === "function") showToast("파일을 불러왔습니다.");
    }catch(err){ console.error(err); if(typeof showToast === "function") showToast("파일을 읽지 못했습니다."); }
  };

  const oldApplyClassicExcerpt = (typeof applyClassicExcerpt === "function") ? applyClassicExcerpt : null;
  window.applyClassicExcerpt = applyClassicExcerpt = function(){
    const inputEl = $("inputText");
    const start = $("excerptStartText") ? $("excerptStartText").value : "";
    const end = $("excerptEndText") ? $("excerptEndText").value : "";
    let next = "";
    if(typeof classicLoadedChunks !== "undefined" && classicLoadedChunks && classicLoadedChunks.length){
      classicActiveChunks = (typeof sliceChunksByStartEnd === "function") ? sliceChunksByStartEnd(classicLoadedChunks, start, end) : classicLoadedChunks.slice();
      next = (typeof chunksDisplayText === "function") ? chunksDisplayText(classicActiveChunks) : classicActiveChunks.map(c=>c.text).join("\n\n");
    }else{
      const base = (typeof classicLoadedFileText !== "undefined" && classicLoadedFileText) || (inputEl ? inputEl.value : "");
      next = (typeof sliceByStartEnd === "function") ? sliceByStartEnd(base, start, end) : String(base || "").trim();
      classicActiveChunks = null;
    }
    if(inputEl) inputEl.value = next;
    classicActiveDisplayText = next;
    renderClassicInputFromText(next, classicActiveChunks);
    robustTransformText();
    if(typeof scheduleAutosave === "function") scheduleAutosave();
  };

  window.loadChatFile = loadChatFile = async function(){
    const fileInput = $("chatFileInput");
    const file = fileInput && fileInput.files && fileInput.files[0];
    if(!file) return;
    try{
      updateFileLabel(fileInput, $("chatFileName"));
      const raw = await readTextFileSafe(file);
      const name = (file.name || "").toLowerCase();
      let html = "";
      if(/\.(mht|mhtml)$/i.test(name) || /MIME-Version:\s*1\.0/i.test(raw)){
        const extracted = (typeof extractHTMLFromMHT === "function") ? extractHTMLFromMHT(raw) : raw;
        html = (typeof sanitizePastedHTML === "function") ? sanitizePastedHTML((typeof extractLikelyChatHTML === "function") ? extractLikelyChatHTML(extracted) : extracted) : extracted;
      }else if(/\.(html|htm)$/i.test(name) || (typeof looksLikeHTML === "function" && looksLikeHTML(raw))){
        html = (typeof sanitizePastedHTML === "function") ? sanitizePastedHTML((typeof extractLikelyChatHTML === "function") ? extractLikelyChatHTML(raw) : raw) : raw;
      }else if(/<!--\s*rofan:/i.test(raw) || /\.(md|markdown)$/i.test(name)){
        html = structuredMarkdownToHtml(raw) || (typeof plainTextToHtml === "function" ? plainTextToHtml(raw) : esc(raw));
      }else{
        html = (typeof plainTextToHtml === "function") ? plainTextToHtml(raw) : esc(raw);
      }
      if(!stripTagsSafe(html).trim()){ if(typeof showToast === "function") showToast("불러온 파일에서 텍스트를 찾지 못했습니다."); return; }
      if(typeof activateChatTab === "function") activateChatTab();
      const paste = $("chatPaste");
      if(paste) paste.innerHTML = html;
      cachedChatFileName = file.name || "저장 파일";
  ...
  window.loadEpubEditFile = loadEpubEditFile = async function(){
    const fileInput = $("epubEditFileInput");
    const file = fileInput && fileInput.files && fileInput.files[0];
    if(!file) return;
    try{
      updateFileLabel(fileInput, $("epubEditFileName"));
      const lower = (file.name || "").toLowerCase();
      let html = "";
      if(/\.epub$/i.test(lower) && typeof extractHtmlFromEpubFile === "function") html = await extractHtmlFromEpubFile(file);
      else {
        const raw = await readTextFileSafe(file);
        if(/\.(html|htm)$/i.test(lower) || (typeof looksLikeHTML === "function" && looksLikeHTML(raw))) html = (typeof sanitizeEditorHTML === "function") ? sanitizeEditorHTML(raw) : raw;
        else if(typeof markdownOrTextToEditorHTML === "function") html = markdownOrTextToEditorHTML(raw, /\.(md|markdown)$/i.test(lower));
        else html = (typeof plainTextToHtml === "function") ? plainTextToHtml(raw) : esc(raw);
      }
      if(!stripTagsSafe(html).trim()){ if(typeof showToast === "function") showToast("불러올 본문을 찾지 못했습니다."); return; }
      if(typeof activateEpubEditTab === "function") activateEpubEditTab();
      const ed = $("epubEditEditor");
      if(ed) ed.innerHTML = html;
      cachedEpubEditFileName = file.name || "원고 파일";
      robustTransformText();
      if(typeof showToast === "function") showToast("원고 파일을 불러왔습니다.");
    }catch(err){ console.error(err); if(typeof showToast === "function") showToast("파일을 읽지 못했습니다."); }
  };

  function bindFileInputsHard(){
    const map = {
      classicFileInput: loadClassicFile,
      chatFileInput: loadChatFile,
      epubEditFileInput: loadEpubEditFile
    };
    Object.keys(map).forEach(id => {
      const inp = $(id);
      if(!inp || inp.dataset.hardeningFileBound) return;
      inp.dataset.hardeningFileBound = "1";
    ...
    [["epubCoverInput","epubCoverFileName","선택 없음"],["epubFontInput","epubFontFileName","선택 없음"]].forEach(([id,labelId,empty])=>{
      const inp=$(id); if(!inp || inp.dataset.hardeningFileBound) return;
      inp.dataset.hardeningFileBound="1";
      const label=inp.closest("label"); if(label){ label.setAttribute("for",id); label.style.position=label.style.position||"relative"; }
      inp.addEventListener("change", () => updateFileLabel(inp, $(labelId), empty), true);
      inp.addEventListener("click", () => { inp.value=""; });
    });
  }

  const oldSyncResultPreview = (typeof syncResultPreview === "function") ? syncResultPreview : null;
  window.syncResultPreview = syncResultPreview = function(el){
    const preview = (el === $("chatOutputText")) ? $("chatOutputTextPreview") : (el === $("outputText") ? $("outputTextPreview") : null);
    if(!preview || !el){ if(oldSyncResultPreview) return oldSyncResultPreview(el); return; }
  ...
  function fallbackRenderClassic(){
    const inputEl = $("inputText");
    const text = inputEl ? inputEl.value : "";
    let chunks = (typeof classicActiveChunks !== "undefined" && classicActiveChunks && classicActiveChunks.length) ? cloneChunks(classicActiveChunks) : null;
    if(!chunks) chunks = safe(() => parseStructuredMarkdownToChunks(text), null) || safe(() => parseChunks(text), []);
    const pack = safe(() => renderChunks(chunks, {removeTables:removeTablesEl && removeTablesEl.checked, deleteTokens:typeof getDeleteContainsTokens === "function" ? getDeleteContainsTokens() : [], deleteContainsMode:deleteContainsModeEl ? deleteContainsModeEl.value : "delete", containsDecisions, dropBlockIds:new Set(), dropChunkIds:new Set(), labelSpeakers:shouldRenderSpeakerLabels(), returnItems:true}), {text:text, items:chunks});
    lastStructuredItems = pack.items || chunks || [];
    setResultOutput($("outputText"), pack.text || "");
    if(typeof updateResultStats === "function") updateResultStats(chunks || [], [], pack.text || "");
  }
  function fallbackRenderChat(){
    const paste = $("chatPaste");
    const parsed = safe(() => parseRofanChatChunks(paste), {chunks:[], blocks:[]});
    const chunks = parsed.chunks || [];
    const pack = safe(() => renderChunks(chunks, {removeTables:removeTablesEl && removeTablesEl.checked, deleteTokens:typeof getDeleteContainsTokens === "function" ? getDeleteContainsTokens() : [], deleteContainsMode:deleteContainsModeEl ? deleteContainsModeEl.value : "delete", containsDecisions, dropBlockIds:new Set(), dropChunkIds:new Set(), labelSpeakers:shouldRenderSpeakerLabels(), returnItems:true}), {text:(paste ? paste.innerText : ""), items:chunks});
    lastStructuredItems = pack.items || chunks || [];
    setResultOutput($("chatOutputText"), pack.text || "");
    if(typeof updateResultStats === "function") updateResultStats(chunks || [], parsed.blocks || [], pack.text || "");
  }

  const oldTransformText = (typeof transformText === "function") ? transformText : null;
  function robustTransformText(){
    const mode = (typeof activeMode === "string" ? activeMode : "classic");
    const inputEl = $("inputText");
    const prev = ensureClassicInputPreview();
    if(mode === "classic" && prev && document.activeElement === prev){
      inputEl.value = (prev.innerText || prev.textContent || "").replace(/\u00a0/g," ").trim();
    }
    let failed = false;
    try{ if(oldTransformText) oldTransformText(); else failed = true; }
    catch(err){ failed = true; console.error("transform failed; fallback used", err); }
    try{
      if(mode === "classic"){
        const out = $("outputText");
        const source = ((inputEl && inputEl.value) || (prev && (prev.innerText || prev.textContent)) || "").trim();
        if(failed || (source && out && !out.value.trim())) fallbackRenderClassic();
        renderClassicInputFromText(inputEl ? inputEl.value : "", (typeof classicActiveChunks !== "undefined" && classicActiveChunks && classicActiveChunks.length) ? classicActiveChunks : null);
      }else if(mode === "chat"){
        const paste = $("chatPaste");
        const out = $("chatOutputText");
        const source = paste ? (paste.innerText || paste.textContent || "").trim() : "";
        if(failed || (source && out && !out.value.trim())) fallbackRenderChat();
      }else if(mode === "epubedit"){
        if(typeof updateResultStats === "function") updateResultStats([], [], $("epubEditEditor") ? ($("epubEditEditor").innerText || "") : "");
      }
      syncResultPreview($("outputText"));
      syncResultPreview($("chatOutputText"));
      updateEpubPreview();
      updateChapterDividerPanel();
      if(typeof syncActiveResultHeight === "function") requestAnimationFrame(syncActiveResultHeight);
    }catch(err){ console.error("post-transform hardening failed", err); }
  }
  window.transformText = transformText = robustTransformText;

  function cleanChapterMarkerText(text){
    return String(text || "").replace(markRe(), "").replace(/\n{3,}/g, "\n\n").trim();
  }
  const oldGetPlainActiveOutputValue = (typeof getPlainActiveOutputValue === "function") ? getPlainActiveOutputValue : null;
  window.getPlainActiveOutputValue = getPlainActiveOutputValue = function(){
    const v = oldGetPlainActiveOutputValue ? oldGetPlainActiveOutputValue() : (typeof getActiveOutputValue === "function" ? getActiveOutputValue() : "");
    return cleanChapterMarkerText(stripTagsSafe(v).replace(/&nbsp;/gi," "));
  };

  function activeRawTextForChapters(){
    if(typeof activeMode !== "undefined" && activeMode === "epubedit"){
      const ed = $("epubEditEditor");
      return ed ? (ed.innerText || ed.textContent || "") : "";
    }
    const out = (typeof activeMode !== "undefined" && activeMode === "chat") ? $("chatOutputText") : $("outputText");
    return out ? (out.value || "") : "";
  }
  function paragraphArrayWithMarkers(text){
    const prepared = String(text || "").replace(markRe(), "\n\n" + CHAPTER_MARK + "\n\n");
    return prepared.split(/\n\s*\n+/).map(p=>p.trim()).filter(Boolean);
  }
  function isChapterMarkerParagraph(p, cfg){
    const raw = String(p || "").trim();
    if(raw === CHAPTER_MARK) return true;
    const clean = raw.replace(/<[^>]*>/g, "").replace(/&nbsp;/gi," ").replace(/\s+/g, "").trim();
    const sep = String((cfg && cfg.separator) || ($("chapterSeparator") && $("chapterSeparator").value) || "—————").replace(/\s+/g, "").trim();
    if(clean && sep && clean === sep) return true;
    return /^[\-—―─━_]{3,}$/.test(clean) && /^[\-—―─━_]{3,}$/.test(sep || "—————");
  }
  function splitTextStable(text, cfg){
    const mode = cfg.chapterMode || "none";
    const paras = paragraphArrayWithMarkers(text);
    const makeTitle = idx => (typeof getChapterTitle === "function" ? getChapterTitle(idx, cfg) : ((cfg.chapterPrefix || "Chapter") + " " + (idx+1)));
    if(!paras.length) return [];
    const chapters = [];
    if(mode === "separator"){
      let bucket = [];
      const flush = () => { const body = bucket.join("\n\n").trim(); if(body) chapters.push({title:makeTitle(chapters.length), body}); bucket = []; };
      paras.forEach(p => { if(isChapterMarkerParagraph(p, cfg)) flush(); else bucket.push(p); });
      flush();
      return chapters.length ? chapters : [{title:makeTitle(0), body:paras.filter(p=>!isChapterMarkerParagraph(p,cfg)).join("\n\n")}];
    }
    if(mode === "paragraphs"){
      const normal = paras.filter(p=>!isChapterMarkerParagraph(p,cfg));
      const n = Math.max(1, cfg.chapterSize || 7000);
      for(let i=0;i<normal.length;i+=n) chapters.push({title:makeTitle(chapters.length), body:normal.slice(i,i+n).join("\n\n")});
      return chapters;
    }
    if(mode === "chars"){
      let bucket=[], count=0, n=Math.max(1, cfg.chapterSize || 7000);
      paras.filter(p=>!isChapterMarkerParagraph(p,cfg)).forEach(p=>{ if(bucket.length && count + p.length > n){ chapters.push({title:makeTitle(chapters.length), body:bucket.join("\n\n")}); bucket=[]; count=0; } bucket.push(p); count += p.length; });
      if(bucket.length) chapters.push({title:makeTitle(chapters.length), body:bucket.join("\n\n")});
      return chapters;
    }
    return [{title:makeTitle(0), body:paras.filter(p=>!isChapterMarkerParagraph(p,cfg)).join("\n\n")}];
  }
  window.splitTextIntoChapters = splitTextIntoChapters = function(text){
    return splitTextStable(text, getEpubConfig());
  };

  function editorBlocksForChapters(){
    const ed = $("epubEditEditor");
    if(!ed) return [];
    const out = [];
    Array.from(ed.childNodes).forEach(node => {
      if(node.nodeType === Node.ELEMENT_NODE && node.matches('hr[data-chapter-separator], hr.chapter-editor-separator, hr')) out.push({sep:true});
      else if((node.textContent || "").trim() || node.nodeType === Node.ELEMENT_NODE) out.push({html:node.outerHTML || esc(node.textContent || "")});
    });
    return out;
  }
  ...
  window.getPreparedOutputForExport = getPreparedOutputForExport = function(){
    if(typeof activeMode !== "undefined" && activeMode === "epubedit"){
      const ed = $("epubEditEditor");
      return ed ? (ed.innerText || ed.textContent || "").replace(markRe(), "").trim() : "";
    }
    const out = (typeof activeMode !== "undefined" && activeMode === "chat") ? $("chatOutputText") : $("outputText");
    let text = out ? (out.value || "") : "";
    const mode = $("epubTextMode") ? $("epubTextMode").value : "current";
    const labelMode = $("speakerLabelMode") ? $("speakerLabelMode").value : "output";
    if((mode === "removeLabels" || labelMode === "hideInEpub" || labelMode === "reviewOnly") && typeof stripSpeakerLabelLines === "function") text = stripSpeakerLabelLines(text);
    if(mode === "novel" && typeof stripSpeakerLabelLines === "function") text = stripSpeakerLabelLines(text).replace(/\n{3,}/g,"\n\n");
    return text.replace(markRe(), "").trim();
  };
  window.getExportChapters = getExportChapters = function(markdownMode){
    const cfg = getEpubConfig();
    if(typeof activeMode !== "undefined" && activeMode === "epubedit"){
      const blocks = editorBlocksForChapters();
      if(markdownMode) return splitTextStable((($('epubEditEditor') && $('epubEditEditor').innerText) || ''), cfg);
      if(cfg.chapterMode === "separator"){
        const chapters=[]; let bucket=[];
        const make = () => { if(bucket.length){ const html=bucket.join("\n"); chapters.push({title:getChapterTitle(chapters.length,cfg), bodyHtml:html, body:textOfHtml(html)}); bucket=[]; } };
        blocks.forEach(b => { if(b.sep) make(); else if(b.html) bucket.push(b.html); }); make();
        return chapters.length ? chapters : (blocks.length ? [{title:getChapterTitle(0,cfg), bodyHtml:blocks.filter(b=>b.html).map(b=>b.html).join("\n"), body:textOfHtml(blocks.filter(b=>b.html).map(b=>b.html).join("\n"))}] : []);
      }
      return splitTextStable((($('epubEditEditor') && $('epubEditEditor').innerText) || ''), cfg);
    }
    return splitTextStable(activeRawTextForChapters(), cfg);
  };
  window.chapterBodyToXhtml = chapterBodyToXhtml = function(ch){
    if(ch && ch.bodyHtml) return (typeof sanitizeEditorHTML === "function" ? sanitizeEditorHTML(ch.bodyHtml) : ch.bodyHtml).replace(/<br>/g,"<br/>").replace(/<hr[\s\S]*?>/gi,"");
    return (typeof textToXhtmlBody === "function") ? textToXhtmlBody((ch && ch.body) || "") : `<p>${esc((ch && ch.body) || "")}</p>`;
  };
  window.updateEpubPreview = updateEpubPreview = function(){
    const chapters = getExportChapters();
    const cfg = getEpubConfig();
    const tocHtml = (typeof renderChapterTocHtml === "function") ? renderChapterTocHtml(chapters, false) : `<div>${chapters.length}개 챕터</div>`;
    if($("chapterPreview")) $("chapterPreview").innerHTML = tocHtml;
    if($("chapterTocInline")) $("chapterTocInline").innerHTML = (typeof renderChapterTocHtml === "function") ? renderChapterTocHtml(chapters, true) : tocHtml;
    if($("epubPreview")){
      const first = chapters[0] || null;
      const body = first ? (first.bodyHtml ? chapterBodyToXhtml(first) : ((typeof splitTextIntoParagraphs === "function") ? splitTextIntoParagraphs(first.body).slice(0,12).map(p => (typeof paragraphToXhtml === "function" ? paragraphToXhtml(p) : `<p>${esc(p)}</p>`)).join("\n") : `<p>${esc(first.body)}</p>`)) : `<p class="mutedText">미리보기 없음</p>`;
      $("epubPreview").innerHTML = `<div class="previewMeta"><b>${esc(cfg.title)}</b>${cfg.author ? `<span>${esc(cfg.author)}</span>` : ""}<span>${chapters.length || 0}개 챕터</span></div><div class="bookPreview">${body}</div>`;
    }
  };

  function activeParagraphMatches(){
    const q = String(chapterKeywordQuery || "").trim().toLowerCase();
    const order = String(chapterOrderQuery || "").trim();
    const arr = paragraphArrayWithMarkers(activeRawTextForChapters());
    let logical = -1;
    let matches = [];
    arr.forEach((p, arrayIndex) => {
      if(isChapterMarkerParagraph(p, getEpubConfig())) return;
      logical += 1;
      matches.push({text:p, index:logical, arrayIndex});
    });
    if(q) matches = matches.filter(p => p.text.toLowerCase().includes(q));
    if(order){ const n=Number(order); if(Number.isFinite(n) && n > 0) matches = matches.filter((p, idx) => idx+1 === n || p.index+1 === n); }
    return matches;
  }
  window.getChapterSearchMatches = getChapterSearchMatches = activeParagraphMatches;
  window.updateChapterDividerPanel = updateChapterDividerPanel = function(){
    const panel = $("chapterDividerPanel"); if(!panel) return;
    currentChapterMatches = activeParagraphMatches();
    const total = currentChapterMatches.length;
    chapterMatchPage = Math.max(0, Math.min(Math.max(0,total-1), chapterMatchPage || 0));
    const current = total ? currentChapterMatches[chapterMatchPage] : null;
    const sep = ($("chapterSeparator") && $("chapterSeparator").value.trim()) || "—————";
    let html = `<div class="miniToolHead"><div><strong>구분선 삽입</strong><span>검색한 지문 위/아래에 챕터 구분선을 넣습니다.</span></div><span class="smallMuted">${total ? `${chapterMatchPage+1}/${total}` : "0개"}</span></div>`;
    if(!current) html += `<div class="emptyState">키워드를 입력하면 해당 문단을 확인하고 구분선을 넣을 수 있습니다.</div>`;
    else html += `<div class="chapterMatchCard"><div class="dupeSummary withNav"><span><span class="pill">문단 ${current.index+1}</span><span class="pill">구분선 ${esc(sep)}</span></span><span class="navButtons"><button type="button" class="navIconBtn" data-chapter-match-page="prev" onclick="window.__chapterNavClick&&window.__chapterNavClick('prev')" ${chapterMatchPage<=0?"disabled":""} aria-label="이전 문단">‹</button><button type="button" class="navIconBtn" data-chapter-match-page="next" onclick="window.__chapterNavClick&&window.__chapterNavClick('next')" ${chapterMatchPage>=total-1?"disabled":""} aria-label="다음 문단">›</button></span></div><div class="previewText fullPreview">${esc(current.text)}</div><div class="chapterInsertRow"><button type="button" class="btn primary" data-chapter-insert="1" onclick="insertChapterDividerAtCurrentMatch()">현재 문단에 구분선 넣기</button></div></div>`;
    panel.innerHTML = html;
  };
  window.insertChapterDividerAtCurrentMatch = insertChapterDividerAtCurrentMatch = function(){
    const matches = activeParagraphMatches();
    if(!matches.length){ if(typeof showToast === "function") showToast("구분선을 넣을 문단을 찾지 못했습니다."); return; }
    const current = matches[Math.max(0, Math.min(matches.length-1, chapterMatchPage || 0))];
    if($("chapterSplitMode")) $("chapterSplitMode").value = "separator";
    const position = $("chapterPosition") ? $("chapterPosition").value : "before";
    if(typeof activeMode !== "undefined" && activeMode === "epubedit"){
      const ed = $("epubEditEditor");
      if(ed){
        const nodes = Array.from(ed.childNodes).filter(n => (n.nodeType === Node.ELEMENT_NODE && n.matches('hr')) || (n.textContent || "").trim());
        const target = nodes[current.arrayIndex] || nodes[current.index];
        const hr = document.createElement("hr"); hr.className="chapter-editor-separator"; hr.dataset.chapterSeparator="true";
        if(target && target.parentNode) target.parentNode.insertBefore(hr, position === "after" ? target.nextSibling : target); else ed.appendChild(hr);
      }
    }else{
      const out = (typeof activeMode !== "undefined" && activeMode === "chat") ? $("chatOutputText") : $("outputText");
      const arr = paragraphArrayWithMarkers(out ? out.value : "");
      const insertAt = position === "after" ? current.arrayIndex + 1 : current.arrayIndex;
      arr.splice(insertAt, 0, CHAPTER_MARK);
      if(out) setResultOutput(out, arr.join("\n\n"));
    }
    updateEpubPreview(); updateChapterDividerPanel();
    if(typeof scheduleAutosave === "function") scheduleAutosave();
    if(typeof showToast === "function") showToast(position === "after" ? "문단 아래에 구분선을 넣었습니다." : "문단 위에 구분선을 넣었습니다.");
  };

  window.buildStandaloneHTML = buildStandaloneHTML = function(){
    const cfg = getEpubConfig();
    const chapters = getExportChapters();
    const toc = chapters.map((ch, idx) => `<li><a href="#ch${idx+1}">${esc(ch.title)}</a></li>`).join("\n");
    const body = chapters.map((ch, idx) => `<section id="ch${idx+1}"><h1>${esc(ch.title)}</h1>${chapterBodyToXhtml(ch)}</section>`).join("\n");
    return `<!doctype html><html lang="${esc(cfg.language)}"><head><meta charset="utf-8"><title>${esc(cfg.title)}</title><style>${typeof getEpubCss === "function" ? getEpubCss() : ""} nav{margin-bottom:2rem;padding:1rem;border:1px solid #ddd;border-radius:12px;} section{max-width:720px;margin:0 auto 3rem;}</style></head><body><nav><strong>목차</strong><ol>${toc}</ol></nav>${body}</body></html>`;
  };

  const tabSettingFields = ["epubTitle","epubSubtitle","epubAuthor","epubSeries","epubVolume","epubDescription","epubTags","epubLanguage","chapterSplitMode","chapterSize","chapterSeparator","chapterTitlePrefix","chapterFirstTitle","chapterKeywordInput","chapterOrderInput","chapterPosition","epubStylePreset","epubTextMode"];
  const tabSettings = window.__rofanTabSettings || (window.__rofanTabSettings = {});
  function saveTabSettings(mode){
    mode = mode || (typeof activeMode === "string" ? activeMode : "classic");
    tabSettings[mode] = tabSettings[mode] || {};
    tabSettingFields.forEach(id => { const el=$(id); if(el) tabSettings[mode][id] = el.value; });
  }
  function loadTabSettings(mode){
    mode = mode || (typeof activeMode === "string" ? activeMode : "classic");
    const data = tabSettings[mode];
    if(!data) return;
    tabSettingFields.forEach(id => { const el=$(id); if(el && Object.prototype.hasOwnProperty.call(data,id)) el.value = data[id]; });
    chapterKeywordQuery = $("chapterKeywordInput") ? $("chapterKeywordInput").value : "";
    chapterOrderQuery = $("chapterOrderInput") ? $("chapterOrderInput").value : "";
  }
  tabSettingFields.forEach(id => { const el=$(id); if(el && !el.dataset.tabSettingBound){ el.dataset.tabSettingBound="1"; el.addEventListener("input", () => { saveTabSettings(); updateEpubPreview(); updateChapterDividerPanel(); }); el.addEventListener("change", () => { saveTabSettings(); updateEpubPreview(); updateChapterDividerPanel(); }); } });
  document.querySelectorAll(".tab").forEach(tab => {
    if(tab.dataset.hardeningTabBound) return;
    tab.dataset.hardeningTabBound = "1";
    tab.addEventListener("click", () => {
      const before = (typeof activeMode === "string" ? activeMode : "classic");
      saveTabSettings(before);
      setTimeout(() => { loadTabSettings(activeMode); robustTransformText(); }, 0);
    }, true);
  });


  // localStorage/IndexedDB가 차단된 브라우저에서도 기본 변환이 멈추지 않게 자동 저장을 안전화합니다.
  function safeLocalGet(key){ try{ return localStorage.getItem(key); }catch(_err){ return null; } }
  function safeLocalSet(key, val){ try{ localStorage.setItem(key, val); return true; }catch(_err){ return false; } }
  function safeLocalRemove(key){ try{ localStorage.removeItem(key); }catch(_err){} }
  window.updateAutosaveBadge = updateAutosaveBadge = function(text){
    const el = document.getElementById("autosaveStatus");
    if(!el) return;
    let suffix = "";
    const raw = (typeof WORK_META_KEY !== "undefined") ? safeLocalGet(WORK_META_KEY) : null;
    if(raw){
      try{
        const meta = JSON.parse(raw);
        if(meta.savedAt){ const d = new Date(meta.savedAt); if(!Number.isNaN(d.getTime())) suffix = ` · ${String(d.getHours()).padStart(2,"0")}:${String(d.getMinutes()).padStart(2,"0")}`; }
        if(meta.fileName) suffix += ` · ${meta.fileName}`;
      }catch(_err){}
    }
    el.textContent = String(text || "") + suffix;
  };
  window.saveWorkNow = saveWorkNow = async function(){
    if(typeof isRestoringWork !== "undefined" && isRestoringWork) return;
    let state = null;
    try{ state = collectWorkValues(); }catch(err){ console.warn("autosave collect failed", err); updateAutosaveBadge("저장 실패"); return; }
    try{
      if(typeof idbPut === "function" && typeof WORK_STATE_KEY !== "undefined") await idbPut(WORK_STATE_KEY, state);
      if(typeof WORK_META_KEY !== "undefined") safeLocalSet(WORK_META_KEY, JSON.stringify({savedAt: state.savedAt, fileName: (typeof cachedChatFileName !== "undefined" ? cachedChatFileName : "") || "", mode: (typeof activeMode !== "undefined" ? activeMode : "classic")}));
      updateAutosaveBadge("자동 저장됨");
    }catch(err){
      if(typeof WORK_FALLBACK_KEY !== "undefined") safeLocalSet(WORK_FALLBACK_KEY, JSON.stringify(state));
      if(typeof WORK_META_KEY !== "undefined") safeLocalSet(WORK_META_KEY, JSON.stringify({savedAt: state.savedAt, mode: (typeof activeMode !== "undefined" ? activeMode : "classic"), fallback:true}));
      updateAutosaveBadge("자동 저장됨");
    }
  };
  window.clearSavedWork = clearSavedWork = async function(){
    if(!window.confirm("저장된 작업 캐시를 삭제하시겠습니까? 새로고침 복원 데이터도 함께 지워집니다.")) return;
    try{ if(typeof idbDelete === "function" && typeof WORK_STATE_KEY !== "undefined") await idbDelete(WORK_STATE_KEY); }catch(_err){}
    if(typeof WORK_META_KEY !== "undefined") safeLocalRemove(WORK_META_KEY);
    if(typeof WORK_FALLBACK_KEY !== "undefined") safeLocalRemove(WORK_FALLBACK_KEY);
    try{ cachedChatFileName = ""; cachedCoverAsset = null; }catch(_err){}
    updateAutosaveBadge("캐시 삭제됨");
    if(typeof showToast === "function") showToast("저장된 작업 캐시를 삭제했습니다.");
  };

  function initHardening(){
    injectHardeningCss();
    ensureClassicInputPreview();
    bindFileInputsHard();
    renderClassicInputFromText($("inputText") ? $("inputText").value : "", null);
    syncResultPreview($("outputText"));
    syncResultPreview($("chatOutputText"));
    updateEpubPreview();
    updateChapterDividerPanel();
  ...
  if(document.readyState === "loading") document.addEventListener("DOMContentLoaded", initHardening);
  else initHardening();
})();

/* chapter button delegation fix */
(function(){
  if(window.__rofanChapterClickDelegationV3) return;
  window.__rofanChapterClickDelegationV3 = true;
  document.addEventListener('pointerdown', function(ev){
    const insert = ev.target && ev.target.closest ? ev.target.closest('button[data-chapter-insert]') : null;
    if(insert){
      // pointerdown에서도 처리해 모바일 스크롤 영역 안 버튼이 click을 잃어도 동작하게 합니다.
      ev.preventDefault();
      ev.stopImmediatePropagation();
      if(typeof insertChapterDividerAtCurrentMatch === 'function') insertChapterDividerAtCurrentMatch();
    }
  }, true);
  document.addEventListener('click', function(ev){
    ...
    const pageBtn = ev.target && ev.target.closest ? ev.target.closest('button[data-chapter-match-page]') : null;
    if(pageBtn){
      ev.preventDefault();
      ev.stopImmediatePropagation();
      const max = Math.max(0, (window.currentChapterMatches || currentChapterMatches || []).length - 1);
      chapterMatchPage = pageBtn.dataset.chapterMatchPage === 'next' ? Math.min(max, (chapterMatchPage || 0) + 1) : Math.max(0, (chapterMatchPage || 0) - 1);
...
try{ restoreSavedWork(); }catch(err){ console.warn("restoreSavedWork skipped", err); }
refreshColorWidgets();
